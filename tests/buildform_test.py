...
def test_three_zibs_form_html_false(yaml_3ZIB_complex, three_zibs_form_expected_html_false):
    result = normalize(buildForm(yaml_3ZIB_complex, False, "cwd", "default", True))
    reg_result = re.findall("<body>(.*)</body>", result)[0] 
    assert reg_result == normalize(three_zibs_form_expected_html_false) #compare body content only (head is not interesting and very long)
    os.remove(os.path.join(os.getcwd(), "3Zorginformatiebouwstenen_(ZIB)_2020.html"))
    os.remove(os.path.join(os.getcwd(), "3Zorginformatiebouwstenen_(ZIB)_2020.pdf"))
